...
def preprocess(self,image):
    self.image=image
    image = cv2.imread(self.image) #convert image to numpy array of shape (h,w,c) -->here c is rgb values
    scaled = cv2.cvtColor(self.image, cv2.COLOR_BGR2GRAY) #convert rgb image to gray scale (h,w) (rgb 24 bits to 8 bits)
...
def gaussian_blur_func(self,image):
    self.image = image
    blur = cv2.GaussianBlur(self.image, (5, 5), 0) #here (5,5) is size of the kernel, can be odd and positive
    return blur
...
def show_images(self,images, titles=None):
    ...
    """
    self.images = images
    assert ((titles is None) or (len(self.images) == len(titles)))
    x= len(self.images)/2
...
def draw_lines(self, image, lines, color=[200, 0, 0], thickness=10):
    """
...
def detect_lane(self,data):
    ...
    gausian_blur = self.gaussian_blur_func(self.gray_image)
    ...
    line_image = self.hough_lines(edge_canny, 1, np.pi/180, 20, 40, 25)
